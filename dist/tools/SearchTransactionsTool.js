    ...
    try {
        ...
        if (input.categoryId && !isUncategorizedFilter) {
            transactions = transactions.filter((t) => {
                // Check main category
                if (t.category_id === input.categoryId)
                    return true;
        ...
        if (input.memo) {
            ...
            transactions = transactions.filter((t) => t.memo && t.memo.toLowerCase().includes(searchMemo));
        }
        // payeeName text search (case-insensitive)
        if (input.payeeName) {
            const searchPayee = input.payeeName.toLowerCase();
            transactions = transactions.filter((t) => t.payee_name && t.payee_name.toLowerCase().includes(searchPayee));
        }
        ...
        if (input.beforeDate)
            filtersApplied.before_date = input.beforeDate;
        if (input.accountId)
            filtersApplied.account_id = input.accountId;
        if (input.categoryId)
            filtersApplied.category_id = input.categoryId;
        if (input.payeeId)
            filtersApplied.payee_id = input.payeeId;
        if (input.minAmount !== undefined)
            filtersApplied.min_amount = input.minAmount;
        if (input.maxAmount !== undefined)
            filtersApplied.max_amount = input.maxAmount;
        if (input.memo)
            filtersApplied.memo = input.memo;
        if (input.payeeName)
            filtersApplied.payee_name = input.payeeName;
        if (input.cleared)
            filtersApplied.cleared = input.cleared;
        if (input.approved !== undefined)
        ...
        if (input.excludeTrackingAccounts)
            filtersApplied.exclude_tracking_accounts = input.excludeTrackingAccounts;
        ...
        return {
            content: [{ type: "text", text: `Error searching transactions: ${error instanceof Error ? error.message : JSON.stringify(error)}` }]
        };
    }
}
