...
import {
  ...
  searchCategoriesInCache,
  syncCategories,
} from "../cache/categories.js";

export const name = "search_categories";
export const description =
  "Search for categories by name (case-insensitive partial match). Searches both category names and group names. Requires sync_categories to be run first, or will auto-sync if cache is empty.";
...
interface SearchCategoriesInput {
  budgetId?: string;
  query: string;
}
...
export async function execute(input: SearchCategoriesInput, api: ynab.API) {
  try {
