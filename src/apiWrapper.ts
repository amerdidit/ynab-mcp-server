 ...
 */
const rateLimitMiddleware: ynab.Middleware = {
  async post(context) {
 ...
 */
export function createTrackedApi(token: string): ynab.API {
 ...
 * Apply middleware to all API sub-objects
 */
function applyMiddleware(api: ynab.API, middleware: ynab.Middleware): void {
  const subApis = [
    "user",
    "budgets",
    "accounts",
  ...
  for (const name of subApis) {
    const subApi = (api as any)[name];
    if (subApi && typeof subApi.withMiddleware === "function") {
      (api as any)[`_${name}`] = subApi.withMiddleware(middleware);
    }
  }
}
