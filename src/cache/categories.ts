...
export interface CachedCategoryGroup {
  id: string;
  name: string;
  hidden: boolean;
  deleted: boolean;
...
export interface CategoriesCache {
  ...
  last_synced: string;
  category_groups: CachedCategoryGroup[];
...
export async function syncCategories(
  ...
  const existing = readCache<CategoriesCache>(budgetId, CATEGORIES_CACHE_FILE);
  ...
  if (existing) {
    ...
    const categoryGroups: CachedCategoryGroup[] = [];
 ...
 */
export function getCategoriesFromCache(budgetId: string): CategoriesCache | undefined {
  return readCache<CategoriesCache>(budgetId, CATEGORIES_CACHE_FILE);
}

/**
 * Get active (non-deleted, non-hidden) categories from cache.
 */
export function getActiveCategories(cache: CategoriesCache): CachedCategory[] {
 ...
 */
export function getActiveCategoryGroups(cache: CategoriesCache): CachedCategoryGroup[] {
 ...
 */
export function searchCategoriesInCache(
  cache: CategoriesCache,
  query: string
): CachedCategory[] {
 ...
 */
export function getCategoriesByGroup(cache: CategoriesCache): {
  group: CachedCategoryGroup;
  categories: CachedCategory[];
}[] {
